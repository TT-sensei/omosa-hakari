...
let maxWeight=1000,step=5,target=0,anim=null,locked=false,currentNavian=-1,answerMode='g';
...
function answer(){if(!locked)return;const gValue=Number($('answer').value);
const kgValue=Number($('answerKg').value);
const value=answerMode==='kg-g'
  ? (Number.isFinite(kgValue)&&Number.isFinite(gValue)?kgValue*1000+gValue:NaN)
  : gValue;
if(!Number.isFinite(value))return;locked=false;$('answer').disabled=true;$('answerKg').disabled=true;$('answerBtn').disabled=true;
